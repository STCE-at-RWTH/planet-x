...
md"""
Using the aptly-named ramp initial condition, the solution ``u(t, x; p)`` will develop one (and exactly one) shock, which is located at
```math
\xi(t; p) = \sqrt{1+(1+p)t}
```
"""
...
We know that ``C_t \leq \left(\max_{x}\left|f'(u(\cdot,x))\right|\right)^{-1}`` for the primal time stepping method, and is likely constrained more by the pushforward.
...
@bind broad_inputs time_stepping_controls_input(
	AbstractRange[0.:0.01:2., 100:10:1990, 0.:0.05:1.],
	Number[0., 1500, 0.5],
	["p", "Nx", "α"],
)
